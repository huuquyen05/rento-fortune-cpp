...
std::string PropertySlot::landOn(Player* player, std::vector<Player*>& allPlayers) {
...
std::string ParkSlot::landOn(Player* player, std::vector<Player*>& allPlayers) {
    // 公园格子触发的事件，例如获得奖金或其他操作
    std::stringstream sstr;
...
std::string SuperTaxSlot::landOn(Player* player, std::vector<Player*>& allPlayers) {
    // 超税格子触发的事件，例如支付超税
    player->updateMoney(-100);  // 假设超税为100
    std::stringstream sstr;
    std::cout << player->getName() << " paid $100 in Super Tax.\n";
    sstr << player->getName() << " paid $100 in Super Tax.";
    return sstr.str();
}
...
std::string LuxuryTaxSlot::landOn(Player* player, std::vector<Player*>& allPlayers) {
    // 奢侈税格子触发的事件，例如支付奢侈税
    player->updateMoney(-75);  // 假设奢侈税为75
    std::stringstream sstr;
    std::cout << player->getName() << " paid $75 in Luxury Tax.\n";
    sstr << player->getName() << " paid $75 in Luxury Tax.";
    return sstr.str();
}
...
std::string GoToJailSlot::landOn(Player* player, std::vector<Player*>& allPlayers) {
    // 进入监狱
    player->goToJail();
    std::stringstream sstr;
    std::cout << player->getName() << " has been sent to Jail.\n";
    sstr << player->getName() << " has been sent to Jail.\n";
    return sstr.str();
}
...
std::string RailwayStationSlot::landOn(Player* player, std::vector<Player*>& allPlayers) {
    if (property->getOwner() == nullptr) {
        // 铁路站未被购买，玩家可以选择购买
        player->buyProperty(property);
    } else {
...
std::string ChanceSlot::landOn(Player* player, std::vector<Player*>& allPlayers) {
    // 机会卡触发的事件，例如玩家抽到机会卡
    std::stringstream sstr;
    ...
    switch (event) {
        case 0:
            ...
            sstr << player->getName() << " advanced to 'Go' and received $200.\n";
            break;
        case 1:
            ...
            sstr << player->getName() << " advanced to Trafalgar Square. If you pass 'Go', you collect $200.\n";
            player->moveto(13);
            break;
        case 2:
            // 前进到 Pall Mall，经过“Go”时，额外获得200美元
            std::cout << player->getName() << " advanced to Pall Mall. If you pass 'Go', you collect $200.\n";
            sstr << player->getName() << " advanced to Pall Mall. If you pass 'Go', you collect $200.\n";
            player->moveto(24);
            break;
        case 3:
            // 前进到最近的公用设施
        ...
        case 5:
            ...
            sstr << player->getName() << " received a dividend of $50 from the bank.\n";
            break;
        case 6:
            // 获取一张"出狱免费卡"（需要实现卡片存储逻辑）
            player->haveoutJailcard();
            std::cout << player->getName() << " received a 'Get out of Jail Free' card.\n";
            sstr << player->getName() << " received a 'Get out of Jail Free' card.\n";
            break;
        case 7:
            std::cout << player->getName() << " goes directly to Jail. (Cannot collect $200 from 'Go')\n";
            sstr << player->getName() << " goes directly to Jail. (Cannot collect $200 from 'Go')\n";
            player->goToJail();  
            break;
        case 8:
            std::cout << player->getName() << " must make general repairs on all properties.\n";
            sstr << player->getName() << " must make general repairs on all properties.\n"; 
        ...
        case 9:
            ...
            sstr << player->getName() << " took a ride to King's Cross Station.\n";
            player->moveto(25);
            break;
        case 10:
            player->updateMoney(-15);  
            std::cout << player->getName() << " paid Poor Tax of $15.\n";
            sstr << player->getName() << " paid Poor Tax of $15.\n"; 
            break;
        case 11:
            std::cout << player->getName() << " advanced to Mayfair.\n";
            sstr << player->getName() << " advanced to Mayfair.\n"; 
            player->moveto(39);
            break;
        case 12:
        ...
        case 13:
            ...
            sstr << player->getName() << " collected $150 from building and loan maturity.\n"; 
            break;
        default:
            std::cout << player->getName() << " drew a Chance card with no effect.\n";
            sstr << player->getName() << " drew a Chance card with no effect.\n";
    }
    return sstr.str();
}
...
std::string UtilitySlot::landOn(Player* player, std::vector<Player*>& allPlayers) {
    if (property->getOwner() == nullptr) {
        // 水电公司未被购买，玩家可以选择购买
        player->buyProperty(property);
    } else {
...
std::string CommunityChestSlot::landOn(Player* player, std::vector<Player*>& allPlayers) {
    ...
    switch (event) {
        ...
        case 6:
            ...
            sstr << player->getName() << " collects $50 from each player for Grand Opera Night.\n"; 
        ...
        case 15:
            ...
            sstr << player->getName() << " won second prize in a beauty contest and received $10.\n"; 
            break;
        case 16:
            player->updateMoney(100);  // 继承100美元
            std::cout << player->getName() << " inherited $100.\n";
            sstr << player->getName() << " inherited $100.\n"; 
            break;
        default:
            std::cout << player->getName() << " drew a Community Chest card with no effect.\n";
            sstr << player->getName() << " drew a Community Chest card with no effect.\n"; 
            break;
    }
    return sstr.str();
}
